...
// FIXME: load the grammar dynamically from resources, but Covfefe can't parse it for some reason,
...
let grammar = try Grammar(abnf: abnf, start: "complete-expression")

try print(String(data: JSONEncoder().encode(grammar), encoding: .utf8)!)
